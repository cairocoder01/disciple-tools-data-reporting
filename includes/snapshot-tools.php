...
class DT_Data_Reporting_Snapshot_Tools
{
    ...
    private static function get_next_period()
    {
        ...
        $period_query = "SELECT period_interval, MAX(period_end) as max_period_end
             FROM $table_name
             GROUP BY period_interval";
        $max_periods = $wpdb->get_results(
            $period_query, // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            ARRAY_A
        );
    ...
    private static function get_snapshots( mixed $name )
    {
        ...
        $snapshots = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM $table_name WHERE period = %s", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                $name
            ),
            ARRAY_A
        );
